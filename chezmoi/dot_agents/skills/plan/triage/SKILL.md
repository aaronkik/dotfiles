...
4. **Grill (if needed).** If the request needs fleshing out, run the `/plan:grilling` and `/design:domain-modeling` skills together — grill it into shape one question at a time, sharpening domain terms and updating `CONTEXT.md`/ADRs inline as decisions land.
